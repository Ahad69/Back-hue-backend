...
// delete Products
  ...
  exports.getPosts = async (req, res) => {
    const { status, code, message, data , totalPost, postPerMonth} = await getApprovedService({
      ...req.query,
    });
    if (data.products) {
      return res.status(code).json({ code, status, message, data , totalPost , postPerMonth });
    }
    res.status(code).json({ code, status, message });
  };
